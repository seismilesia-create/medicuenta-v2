...
export function buildSystemPromptPaciente(opts: { config: ConfigAgente | null; contactName?: string }): string {
  ...
  return [
    ...
    `- Si reservar_turno te avisa que la obra social está SUSPENDIDA: transmitile el aviso tal cual (la consulta sería particular, se abona en el consultorio) y preguntale si quiere reservar igual. SOLO si acepta, volvé a llamar con os_confirmada:"si" manteniendo la obra social ORIGINAL que dijo el paciente — JAMÁS la cambies vos a "particular": el médico necesita ver la OS real en su agenda.`,
    `- SOLO podés decir que una obra social está suspendida si reservar_turno te lo avisó EN ESTE MISMO TURNO para ESA obra social exacta. Jamás lo afirmes por tu cuenta ni lo generalices a otras obras sociales.`,
