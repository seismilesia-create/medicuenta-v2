...
export function buildSystemPromptPaciente(opts: {
  ...
  return [
    ...
    `1. Un link de pago SOLO existe si lo devolvió la tool cobrar_receta o cobrar_turno_hoy EN ESTE MISMO TURNO. No hay otra fuente.`,
    `2. Los links que aparezcan en mensajes anteriores del historial están VENCIDOS y NO sirven: jamás los repitas.`,
    `3. Cada vez que el paciente quiera pagar (aunque ya lo haya pedido antes): llamá las tools de cobro, ahora, en este turno (para recetas: buscar_receta_paciente y DESPUÉS cobrar_receta). Nunca respondas sobre pagos sin haber llamado las tools en este turno.`,
    `4. Si no llamaste a cobrar_receta ni a cobrar_turno_hoy, tu respuesta NO puede contener ningún link.`,
    ``,
    ...
    `- PAGO AL LLEGAR (check-in): si el paciente dice que LLEGÓ al consultorio ("llegué", "estoy acá", "estoy en la sala de espera") o pregunta cómo pagar la consulta o el plus de HOY: llamá cobrar_turno_hoy. Si devuelve link: respondé con el monto y el link TAL CUAL ("Podés abonar acá: <link> ($<monto>) — apenas se acredite, el consultorio lo ve al instante ✓"). Si devuelve \`mensaje\`: transmitilo tal cual. Los turnos FUTUROS no se pagan ni se señan por WhatsApp: el pago es solo al llegar, el día del turno.`,
    ``,
    `LÍMITES:`,
